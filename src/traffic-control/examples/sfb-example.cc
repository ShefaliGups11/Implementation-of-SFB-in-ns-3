...
NS_LOG_COMPONENT_DEFINE ("SFBExample");
...
{
  uint32_t qSize = StaticCast<SfbQueueDisc> (queue)->GetQueueSize ();
...
{
  LogComponentEnable ("SfbQueueDisc", LOG_LEVEL_INFO);

  std::string SfbLinkDataRate = "1.5Mbps";
  std::string SfbLinkDelay = "20ms";
  ...
  // Sfb params
  NS_LOG_INFO ("Set Sfb params");
//  Config::SetDefault ("ns3::SfbQueueDisc::Mode", StringValue ("QUEUE_DISC_MODE_PACKETS"));
  Config::SetDefault ("ns3::SfbQueueDisc::MeanPktSize", UintegerValue (meanPktSize));
  Config::SetDefault ("ns3::SfbQueueDisc::QueueLimit", UintegerValue (100));
  ...
  TrafficControlHelper tchSfb;
  tchSfb.SetRootQueueDisc ("ns3::SfbQueueDisc");
  ...
  p2p.SetQueue ("ns3::DropTailQueue");
  p2p.SetDeviceAttribute ("DataRate", StringValue (SfbLinkDataRate));
  p2p.SetChannelAttribute ("Delay", StringValue (SfbLinkDelay));
  devn2n3 = p2p.Install (n2n3);
  // only backbone link has Sfb queue disc
  queueDiscs = tchSfb.Install (devn2n3);
  ...
  if (writeForPlot)
    {
      filePlotQueueDisc << pathOut << "/" << "Sfb-queue-disc.plotme";
      filePlotQueueDiscAvg << pathOut << "/" << "Sfb-queue-disc_avg.plotme";
  ...
  if (st.GetNDroppedPackets (SfbQueueDisc::OVERLIMIT_DROP) != 0)
    {
  ...
  if (printSfbStats)
    {
      std::cout << "*** Sfb stats from Node 2 queue ***" << std::endl;
      std::cout << "\t " << st.GetNDroppedPackets (SfbQueueDisc::TARGET_EXCED_DROP)
                << " drops due to TARGET_EXCED_DROP " << std::endl;
      std::cout << "\t " << st.GetNDroppedPackets (SfbQueueDisc::OVERLIMIT_DROP)
                << " drops due to OVERLIMIT_DROP " << std::endl;
      std::cout << "\t " << st.GetNDroppedPackets (SfbQueueDisc::RATELIMIT_DROP)
                << " drops due to RATELIMIT_DROP" << std::endl;
      std::cout << "\t " << st.GetNDroppedPackets (SfbQueueDisc::BUCKETLIMIT_DROP)
                << " drops due to BUCKETLIMIT_DROP" << std::endl;
    }
